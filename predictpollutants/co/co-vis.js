...
async function main() {
    ...
    if (trainData !== null) {
        const toDisplay = trainingDataJSON.map(item => ({
            x: item.co_sub_index, y: item.psi_twenty_four_hourly
        }));
        const surface1 = { name: 'Carbon Monoxide', tab: 'Value Distribution' };
        const surface3 = { name: 'Carbon Monoxide', tab: 'Value Prediction' };
