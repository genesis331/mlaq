...
async function main() {
    ...
    if (trainData !== null) {
        const toDisplay = trainingDataJSON.map(item => ({
            x: item.so2_sub_index, y: item.psi_twenty_four_hourly
        }));
        const surface1 = { name: 'Sulfur Dioxide', tab: 'Value Distribution' };
        const surface3 = { name: 'Sulfur Dioxide', tab: 'Value Prediction' };
