...
async function main() {
    ...
    if (trainData !== null) {
        const toDisplay = trainingDataJSON.map(item => ({
            x: item.no2_one_hour_max, y: item.psi_twenty_four_hourly
        }));
        const surface1 = { name: 'Nitrogen Dioxide', tab: 'Value Distribution' };
        const surface3 = { name: 'Nitrogen Dioxide', tab: 'Value Prediction' };
