...
async function main() {
    ...
    if (trainData !== null) {
        const toDisplay = trainingDataJSON.map(item => ({
            x: item.o3_sub_index, y: item.psi_twenty_four_hourly
        }));
        const surface1 = { name: 'Ground-level Ozone', tab: 'Value Distribution' };
        const surface3 = { name: 'Ground-level Ozone', tab: 'Value Prediction' };
